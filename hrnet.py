...
def transition_up_fuse(layer1,layer2,filters,target_size=0,batch_norm = True,activation_fun = False):
    ...
    layer1 = UpSampling2D(size = int(ratio),interpolation='bilinear')(layer1)
...
def upscale(layer,filters,target_size=0,batch_norm = True,activation_fun = False,aspect_ratio=None):
    if aspect_ratio is None:
         size = layer.shape[1]
         ratio = target_size/size
         layer = UpSampling2D(size = int(ratio),interpolation='bilinear')(layer)
    else:
        layer = UpSampling2D(size = aspect_ratio,interpolation='bilinear')(layer)
...
def block_1(input_layer):
    ...
    out=resolution_block(out,1,256,3,activ_relu=True)
    return out
...
def first_block(input_layer):   #input_layer,filters,kernel_size=3,strides=(2,2),batch_norm=False,activ_relu=False
    x=downsample_layer(input_layer,64,strides=(1,2),batch_norm=True,activ_relu=True)
    out1=block_1(x)
    out2=resolution_block(x,1,256,3,activ_relu=True)  #8irt hna dlw2ty
...
def block2_1(input_layer):
    ...
    out=resolution_block(out,1,32,3)  #conv>bn>act>con>bn
    return out
...
def block2_2(input_layer,filters=64):
    ...
    out=resolution_block(out,1,filters,3)
    return out
...
def build_hrnet(input_shape = (256,512,3),n_classes=30,weights = None):
  ...
  #building convc of low resolution
    ...
    #upscaling very low
    output_1=upscale(out_4_verylow_f,32,target_size=256,batch_norm = True,activation_fun =True)  #vlow
    output_2=upscale(out_4_low,32,target_size=256,batch_norm = True,activation_fun =True)  #low
    output_3=upscale(out_18,32,target_size=256,batch_norm = True,activation_fun = True)   #medium
   ...
   # out=[out_5,out_8,out_10,out_11,out_12,out_13,out_14,out_15,out_16,out_17,out_18,out_19,out_20,out_21,out_4]
    final_out=upscale(out_f,filters=n_classes,aspect_ratio=(1,2),batch_norm = True,activation_fun = False)
